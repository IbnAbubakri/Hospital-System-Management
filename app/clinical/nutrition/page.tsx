...
export default function NutritionPage() {
  ...
  const columns = [
    { title: 'Patient', key: 'patient', render: (_: any, r: NutritionPlan) => (<div><div className="font-medium">{r.patientName}</div><Text type="secondary" className="">{r.mrn}</Text></div>) },
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, #F0F9FF 0%, #F8FAFC 100%)' }}>
      ...
      <div style={{ padding: '32px', background: 'linear-gradient(135deg, #FFFFFF 0%, #F8FAFC 100%)', borderBottom: '1px solid #E2E8F0' }}>
        <div className="page-content">
          <div className="   ">
            <div className="  "><div style={{ width: '4px', height: '28px', background: 'linear-gradient(180deg, #3B82F6 0%, #2563EB 100%)', borderRadius: '2px' }} /><div><h1 className="text-2xl font-semibold ">Nutrition Management</h1></div></div>
            <Button type="primary" icon={<PlusOutlined />}>New Plan</Button>
          </div>
          <p className=" " style={{ marginLeft: '7px' }}>Manage patient nutrition plans and dietary requirements</p>
      ...
      <div className="  page-content">
        <Card title="Nutrition Plans" extra={<div className=" "><Search placeholder="Search..." allowClear style={{ width: 200 }} value={searchText} onChange={(e) => setSearchText(e.target.value)} prefix={<SearchOutlined />} /><Select placeholder="Status" value={statusFilter} onChange={setStatusFilter} allowClear style={{ width: 140 }}><Option value="active">Active</Option><Option value="pending_review">Pending</Option><Option value="modified">Modified</Option></Select></div>}>
      ...
      <Drawer title="Nutrition Plan Details" placement="right" size="large" open={drawerVisible} onClose={() => { setDrawerVisible(false); setSelectedPlan(null); }}>
        {selectedPlan && (<Card><div className=""><Text type="secondary">Patient</Text><div className="text-xl font-semibold">{selectedPlan.patientName}</div><div className=" ">{selectedPlan.mrn}</div></div><Row gutter={16}><Col span={12}><div className=""><Text type="secondary">Diet Type</Text><div>{selectedPlan.dietType}</div></div></Col><Col span={12}><div className=""><Text type="secondary">Status</Text><div><Tag color="success">ACTIVE</Tag></div></div></Col><Col span={12}><div className=""><Text type="secondary">Calorie Target</Text><div>{selectedPlan.calorieTarget} kcal</div></div></Col><Col span={12}><div className=""><Text type="secondary">Protein Target</Text><div>{selectedPlan.proteinTarget}g</div></div></Col></Row><div className=""><Text type="secondary">Dietician</Text><div>{selectedPlan.dietician}</div></div></Card>)}
      </Drawer>
    </div>
  );
}
