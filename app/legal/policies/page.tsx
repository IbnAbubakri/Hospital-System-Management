...
import { Card, Typography, Table, Tag, Space, Button, Input, Select, DatePicker, App } from 'antd';
...
export default function PoliciesPage() {
  ...
  // Handle view policy
  const handleViewPolicy = (record: any) => {
    message.success(`Viewing policy: ${record.name}`);
  };

  // Handle download policy
  const handleDownloadPolicy = (record: any) => {
    message.success(`Downloading policy: ${record.name}`);
  };
  ...
  const columns = [
    ...
    { title: 'Next Review', dataIndex: 'requiredReview', key: 'requiredReview' },
    {
      ...
      render: (_: any, record: any) => (
        <Space>
          <Button type="link" size="small" onClick={() => handleViewPolicy(record)}>View</Button>
          <Button type="link" size="small" onClick={() => handleDownloadPolicy(record)}>Download</Button>
        </Space>
      ),
    },
  ];
